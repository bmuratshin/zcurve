...
CRandomMT::CRandomMT (uint32_t _seed):left (-1), seedValue (_seed)
{
  SeedMT (seedValue);
}

void
CRandomMT::SeedMT (uint32_t seed)
{
  //
...
main (int argc, const char *argv[])
{
  ...
  for (int i = 0; i < cnt; )
    {
      double x = rand.RandomMax (lmax);
      double y = rand.RandomMax (lmax);
      double z = rand.RandomMax (lmax);
